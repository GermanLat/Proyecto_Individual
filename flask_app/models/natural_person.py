...
table_name="natural_persons"

class Natural_Person:
    def __init__(self, data):
        self.id = data['id']
        self.identity_document = data['identity_document']
    ...
    def validate_B(cls, user_D):
        ...
        if len(user_D['identity_document']) < 4 or not user_D['identity_document'].isalnum():
            flash("Identity document number must have at least 4 characters and all alphanumeric characters.", "register")
            is_valid = False
        if len(user_D['first_name']) < 2 or not user_D['first_name'].isalpha():
            flash("First name must have at least 2 characters and all alphabetic characters.", "register")
            is_valid = False
        if len(user_D['last_name']) < 2 or not user_D['last_name'].isalpha():
            flash("Last name must be at least 2 characters and all alphabetic characters.", "register")
            is_valid = False
    ...
    def save_N(cls, data_D):
        print(f'Inicio del módulo {inspect.stack()[0][3]}() \t- \tClass: {cls.__name__}')
        query = "INSERT INTO " + table_name + " (identity_document, first_name, last_name, created_at, updated_at) \
            VALUES (%(identity_document)s, %(first_name)s, %(last_name)s, NOW(), NOW());"
    ...
    # def update(cls,data):
    #     print(f'Inicio del módulo {inspect.stack()[0][3]}() \t- \tClass: {cls.__name__}')
    #     query = "UPDATE " + table_name + " SET identity_document=%(identity_document)s, \
    #         first_name=%(first_name)s, last_name=%(last_name)s, updated_at=NOW() WHERE id = %(id)s;"
    #     print(f'Fin del módulo {inspect.stack()[0][3]}() \t- \tClass: {cls.__name__}')
